...
print("D&A vs CapEx source data:")
print(chart_data_df.to_string(index=False))

figure, axis = plt.subplots(figsize=(9, 5.5))
da_line = axis.plot(
    chart_data_df["Fiscal Year"],
    chart_data_df["Depreciation & Amortization"],
    marker="o",
    label="Depreciation & Amortization",
)[0]
capex_line = axis.plot(
    chart_data_df["Fiscal Year"],
    chart_data_df["Capital Expenditures"],
    marker="o",
...
chart_path = chart_directory / "da_vs_capex.png"
...
for column in ["Depreciation & Amortization", "Capital Expenditures"]:
    try:
...
financial_lines = [da_line, capex_line]
expected_labels = ["Depreciation & Amortization", "Capital Expenditures"]
...
if validation_errors:
    raise ValueError(
        "D&A vs CapEx chart validation failed:\n- "
        + "\n- ".join(validation_errors)
    )

print("D&A vs CapEx chart saved successfully")
print(chart_path.relative_to(project_root).as_posix())
print("D&A vs CapEx chart validated successfully")
