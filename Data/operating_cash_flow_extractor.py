"""Dedicated Operating Cash Flow extraction from Task 72 annual SEC data."""
...
OPERATING_CASH_FLOW_METRIC = "Operating Cash Flow"
_OCF_DEFINITION = next(
    definition
    for definition in FINANCIAL_FIELD_DEFINITIONS
    if definition.name == OPERATING_CASH_FLOW_METRIC
)
VALID_OPERATING_CASH_FLOW_CONCEPTS = frozenset(
    (candidate.taxonomy, candidate.concept)
    for candidate in _OCF_DEFINITION.concepts
)


@dataclass(frozen=True)
class OperatingCashFlowExtraction(ProvenanceMixin):
    metric_name: str
    company_name: str
    ticker: str
    raw_sec_value: Any
...
def extract_operating_cash_flow(
    ...
    matches = tuple(
        ...
        and value.financial_field == OPERATING_CASH_FLOW_METRIC
    )
    if not matches:
        return OperatingCashFlowExtraction(
            metric_name=OPERATING_CASH_FLOW_METRIC,
            ...
            validation_reason=(
                "No annual Operating Cash Flow record exists for the requested "
                "fiscal year."
            ),
        )
    if len(matches) > 1:
        return OperatingCashFlowExtraction(
            ...
            validation_reason=(
                "Multiple Operating Cash Flow records exist for the same fiscal "
                "year; no value was selected."
            ),
        )
    ...
    concept_is_valid = (
        fact.xbrl_taxonomy,
        fact.xbrl_concept,
    ) in VALID_OPERATING_CASH_FLOW_CONCEPTS
    ...
    if status is FactStatus.RETRIEVED and not concept_is_valid:
        status = FactStatus.NEEDS_VALIDATION
        reason = (
            "The selected fact does not use an approved annual Operating Cash "
            "Flow concept."
        )
    elif status is FactStatus.RETRIEVED and not can_normalize:
        status = FactStatus.NEEDS_VALIDATION
        reason = (
            "Operating Cash Flow was reported, but it cannot be normalized to "
            "USD millions without estimation or currency conversion."
        )
