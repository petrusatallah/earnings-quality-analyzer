"""Dedicated Revenue extraction from Task 72 annual SEC filing data."""
...
_REVENUE_DEFINITION = next(
    definition
    for definition in FINANCIAL_FIELD_DEFINITIONS
    if definition.name == REVENUE_METRIC
)
VALID_REVENUE_CONCEPTS = frozenset(
    (candidate.taxonomy, candidate.concept)
    for candidate in _REVENUE_DEFINITION.concepts
)


@dataclass(frozen=True)
class RevenueExtraction(ProvenanceMixin):
    metric_name: str
    company_name: str
    ticker: str
    raw_sec_value: Any
...
def extract_revenue(
    ...
    """Extract one annual Revenue fact without fetching, estimating, or mutation."""
    ...
    matches = tuple(
        ...
        and value.financial_field == REVENUE_METRIC
    )
    if not matches:
        return RevenueExtraction(
            ...
            validation_reason="No annual Revenue record exists for the requested fiscal year.",
        )
    if len(matches) > 1:
        return RevenueExtraction(
            metric_name=REVENUE_METRIC,
            ...
            validation_reason=(
                "Multiple Revenue records exist for the same fiscal year; "
    ...
    concept_is_valid = concept_key in VALID_REVENUE_CONCEPTS
    ...
    if status is FactStatus.RETRIEVED and not concept_is_valid:
        status = FactStatus.NEEDS_VALIDATION
        reason = "The selected fact does not use an approved annual Revenue concept."
    elif status is FactStatus.RETRIEVED and not can_normalize:
        status = FactStatus.NEEDS_VALIDATION
        reason = (
            "Revenue was reported, but it cannot be normalized to USD millions "
