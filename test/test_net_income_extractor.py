"""Live validation tests for Task 74 Net Income extraction."""
...
from Data.net_income_extractor import (  # noqa: E402
    VALID_NET_INCOME_CONCEPTS,
    extract_net_income,
)
...
def test_task_74_live_net_income_extraction() -> None:
    ...
    for ticker in LIVE_TICKERS:
        ...
        net_income_facts = tuple(
            value
            for value in statements.values
            if value.financial_field == "Net Income"
            and value.status is FactStatus.RETRIEVED
        )
        assert net_income_facts
        latest_fiscal_year = max(value.fiscal_year for value in net_income_facts)
        source_fact = next(
            value
            for value in net_income_facts
        ...
        assert result.metric_name == "Net Income"
        ...
        print(f"Net Income raw SEC value: {result.raw_sec_value}")
    ...
    print("Task 74 Net Income extraction tests passed")


if __name__ == "__main__":
    test_task_74_live_net_income_extraction()
