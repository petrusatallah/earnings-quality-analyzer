"""Live validation tests for Task 75 Operating Cash Flow extraction."""
...
from Data.operating_cash_flow_extractor import (  # noqa: E402
    VALID_OPERATING_CASH_FLOW_CONCEPTS,
...
def test_task_75_live_operating_cash_flow_extraction() -> None:
    ...
    for ticker in LIVE_TICKERS:
        ...
        ocf_facts = tuple(
            ...
            if value.financial_field == "Operating Cash Flow"
            and value.status is FactStatus.RETRIEVED
        )
        assert ocf_facts
        latest_fiscal_year = max(value.fiscal_year for value in ocf_facts)
        source_fact = next(
            value
            for value in ocf_facts
            if value.fiscal_year == latest_fiscal_year
        )
        raw_value_before_extraction = source_fact.raw_value

        result = extract_operating_cash_flow(
            identity_result.company,
            latest_fiscal_year,
            statements,
        )
        ...
        assert result.metric_name == "Operating Cash Flow"
        ...
        print(f"Operating Cash Flow raw SEC value: {result.raw_sec_value}")
    ...
    print("Task 75 Operating Cash Flow extraction tests passed")


if __name__ == "__main__":
    test_task_75_live_operating_cash_flow_extraction()
